<script>
	export default {
		onLaunch: function() {
			let version = "1.4.0"
			let oldVersion = uni.getStorageSync('version') || ''
			// 处理上个版本和这个版本缓存不一致的内容
			if (version != oldVersion) {
				uni.removeStorageSync('chat')
				uni.setStorageSync('version', version)
			}
			console.log(
				`\n %c 云AI助手V${version} %c 公众号：南羽工作室 \n\n`,
				"color: #ffffff; background: #3c9cff; padding:5px 0;",
				"color: #3c9cff;background: #ffffff; padding:5px 0;"
			);
			uni.hideTabBar();
			this.$store.commit('getPlatform')
			// this.$store.dispatch('getUserInfo')
			// 获取基本配置
			this.getTabbar();
			this.getLoginConfig();
			this.getRobot();
			this.getModelList();
			this.getAd();

		},
		methods: {
			// 获取底部tabbar
			async getTabbar() {
				const {
					data
				} = await uni.$u.http.post("/app/renovation/tabbar/list");
				this.$store.commit('setTabbar', data.data?.filter(item => item.platform === this.$store.state
					.platform || item
					.platform === 0) || [])
				if (uni.$u.page() === '/pages/index') {
					this.$store.commit('setTabbarIndex', 0)
					data?.data && uni.$u.route(data?.data?.[0]?.pagePath)
				}
			},
			// 获取登录配置
			async getLoginConfig() {
				const {
					data
				} = await uni.$u.http.get("/app/application/info/loginConfig");
				this.$store.commit('setLoginConfig', data.data)
			},
			// 获取AI配置
			async getRobot() {
				const {
					data
				} = await uni.$u.http.get("app/robot/info/info", {
					params: {
						id: 1,
					},
				});
				this.$store.commit('setRobot', data.data)
			},
			// 获取广告
			async getAd() {
				const {
					data
				} = await uni.$u.http.post("/app/ad/info/list");
				data.data.forEach(item => {
					// 处理富文本转义的问题
					item.desc = item.desc?.replace(
						/\<p\>&lt;ad(.*?)&gt;&lt;\/ad&gt;\<\/p\>/g,
						'<ad$1></ad>'
					);
				})
				uni.setStorageSync("ad", data.data?.filter(item => item.platform === this.$store.state.platform || item
					.platform === 0) || []);
			},
			// 获取模型
			async getModelList() {
				const {
					data
				} = await uni.$u.http.post('/app/chatgpt/model/list')
				this.$store.commit('setModelList', data.data)
				if (!this.$store.state.model.id) {
					this.$store.commit('setModel', data.data?.[0] || {})
				}
			}
		}
	};
</script>

<style lang="scss">
	/* 注意要写在第一行，同时给style标签加入lang="scss"属性 */
	@import "uview-ui/index.scss";
	@import "style/common.scss";
	@import "style/theme.scss";

	page {
		height: 100%;
		font-size: 28rpx;
		font-family: "PingFang SC", "Helvetica Neue", Helvetica, Arial, sans-serif;
	}
</style>